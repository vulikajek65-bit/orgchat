import { Camera, ContactRound, LoaderCircle, MessageCircle, UserPlus, X } from 'lucide-react';
...
interface ProfileModalProps {
  ...
  onAddMemberToContacts: (member: MemberWithProfile) => Promise<void>;
...
export const ProfileModal = ({
  ...
  const isAlreadyContact = Boolean(
    !isOwnProfile && member && contacts.some((contact) => contact.linked_profile_id === member.user_id),
  );
  ...
  const addCurrentMemberToContacts = async () => {
    ...
    setError('');
    try {
      await onAddMemberToContacts(member);
    } catch (contactError) {
      console.error('Failed to add member to contacts.', contactError);
  ...
  const startDirectChat = async () => {
    ...
    setError('');
    try {
      await onStartDirectChat(member.user_id);
      onClose();
    } catch (chatError) {
      console.error('Failed to start direct chat.', chatError);
      setError(chatError instanceof Error ? chatError.message : 'Не удалось открыть личный диалог.');
    } finally {
      setContactActionLoading(false);
    }
  };
          ...
          <aside className="rounded-lg bg-slate-50 p-4">
            ...
            {!isOwnProfile ? (
              <div className="mt-4 grid gap-2">
                <button
                  className="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-brand-600 px-3 py-2 text-sm font-medium text-white disabled:opacity-60"
                ...
                <button
                  className="inline-flex w-full items-center justify-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-medium disabled:opacity-60"
                  disabled={contactActionLoading || isAlreadyContact}
                  onClick={() => void addCurrentMemberToContacts()}
                  type="button"
                >
                  <UserPlus className="h-4 w-4" />
                  {isAlreadyContact ? 'Уже в контактах' : 'Добавить в контакты'}
                </button>
              </div>
            ) : null}
          </aside>
                ...
                <div className="mt-4 space-y-2">
                  {visibleContacts.length ? (
                    visibleContacts.map((contact) => (
                      <article className="flex items-center justify-between gap-3 rounded-lg bg-slate-50 p-3 text-sm" key={contact.id}>
                        <div className="min-w-0">
                          <p className="truncate font-medium">{contact.contact_name}</p>
                          <p className="truncate text-slate-500">
                            {contact.linked_profile_id
                              ? 'Контакт OrgChat'
                              : contact.phone || contact.email || 'Без телефона и email'}
                          </p>
                        </div>
                        <button
                          className="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 font-medium disabled:cursor-not-allowed disabled:opacity-50"
                          disabled={!contact.linked_profile_id}
                          onClick={() =>
                            contact.linked_profile_id && void onStartDirectChat(contact.linked_profile_id)
                          }
                          type="button"
                        >
                          <MessageCircle className="h-4 w-4" />
