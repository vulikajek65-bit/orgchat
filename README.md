...
- Голосовые сообщения и вложения: фото, видео, аудио, документы и файлы.
- Личные диалоги между сотрудниками и быстрый старт переписки из контактов.
...
7. Затем выполните [`supabase/media-upgrade.sql`](./supabase/media-upgrade.sql).
   Если редактор Supabase режет большие вставки, используйте короткие файлы `media_upgrade_part_1.sql` ... `media_upgrade_part_4.sql` по порядку.
8. Затем выполните [`supabase/direct-messages-upgrade.sql`](./supabase/direct-messages-upgrade.sql).
   Если редактор Supabase режет большие вставки, используйте короткие файлы `direct_messages_upgrade_part_1.sql` ... `direct_messages_upgrade_part_3.sql` по порядку.
9. В `Authentication` включите Email provider.
10. Для первого локального MVP откройте `Authentication -> Providers -> Email` и отключите `Confirm email`. Если подтверждение email включено, после регистрации пользователь увидит сообщение `Проверьте email для подтверждения аккаунта`.
11. В `Database > Replication` включите realtime для таблицы `messages`, если он не включен автоматически. Для `notifications` публикация добавляется миграцией.
12. Скопируйте `Project URL` и `anon public key` в `.env.local`.
...
- `supabase/media-upgrade.sql` - таблица вложений и storage bucket `message-attachments`.
- `supabase/direct-messages-upgrade.sql` - RLS-политики для личных диалогов.
...
- Приватные вложения через signed URLs вместо публичного MVP-bucket.
