...
8. В `Authentication` включите Email provider.
9. Для первого локального MVP откройте `Authentication -> Providers -> Email` и отключите `Confirm email`. Если подтверждение email включено, после регистрации пользователь увидит сообщение `Проверьте email для подтверждения аккаунта`.
10. В `Database > Replication` включите realtime для таблицы `messages`, если он не включен автоматически. Для `notifications` публикация добавляется миграцией.
11. Скопируйте `Project URL` и `anon public key` в `.env.local`.
